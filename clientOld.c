...
void deRegister(int sock, char* cliName, struct sockaddr* servAddr, char* sendBuf, CC reg);

//function to connect a client and allow for chat
void connectToClient(CC reg, char* sendBuf, char* recvBuf, 
   struct sockaddr_in servAddr, struct sockaddr_in cliAddr,
   int UDPsocket, char* name);
...
int main(int argc, char* argv[]){
   ...
   if(argc != 2){
      printf("ERROR: You must enter 1 name on the command line.\n");
      return EXIT_FAILURE;
   }
   ...
   connectToClient(reg, sendBuf, recvBuf, servAddr, cliAddr, UDPsocket, argv[1]);
   return EXIT_SUCCESS;
}
...
void connectToClient(CC reg, char* sendBuf, char* recvBuf, 
   ...
   if(sockTCP < 0){ //if < 0 we have an error
      deRegister(UDPsocket, name, (struct sockaddr*) &servAddr, sendBuf, reg);
   ...
   if(waitOrMake == 'm'){ //if the client wants to make a connection
      ...
      //have to work out who made the connection
      connectee = getName(reg, &cliOfInterest);
      printf("%s.\n", connectee);
   }
   ...
   //degregister now that we are offline
   deRegister(UDPsocket, name, (struct sockaddr*) &servAddr, sendBuf, reg);
   return;
}
...
void deRegister(int sock, char* cliName, struct sockaddr* servAddr, char* sendBuf, CC reg){
   ...
   clearList(reg); //free the memory that the LL used
